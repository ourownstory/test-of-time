...
try:
    ...
    raise ImportError("The LinearRegression model could not be imported.")

try:
    from darts.models import NBEATSModel, RegressionModel
...
class DartsTorchModel(Model):
    """
    A forecasting model using a torch model from the darts library.
    """

    model_class: PastCovariatesTorchModel
    ...
    def __post_init__(self):
        ...
        self.model = self.model_class(**model_params)
        self.n_forecasts = self.params["n_forecasts"]
        self.n_lags = model_params["input_chunk_length"]
        # input checks are provided by model itself
...
@dataclass
class NbeatsModel(DartsTorchModel):
    """
    A forecasting model using a random forest to obtain a forecast.
    ...
    >>>     (
    >>>         NbeatsModel,
    >>>         {"input_chunk_length ": 12, "output_chunk_length ": 4},
    >>>     ),
    >>> ]
    >>>
    >>> benchmark = SimpleBenchmark(
    ...
    model_name: str = "NbeatsModel"
    model_class: Type = NBEATSModel
