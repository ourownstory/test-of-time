...
from experiments.evaluation.baseline_forecasts.ar_series import calc_ar_series
...
def gen_one_shape_ar(
    ...
    for group_num in range(len(n_ts_groups)):  # looping over two groups
        ...
        for i in range(n_ts):
            ...
            if calc_without_noise:
                ...
                combined_data_without_noise = proportion_season * season_group[i] + proportion_ar * ar_group_without_noise[i]
                # mean_no_noise = np.mean(combined_data)
                # std_dev_no_noise = np.std(combined_data)
                combined_data_without_noise = (combined_data_without_noise - mean) / std_dev
                df["y_ideal"] = combined_data_without_noise * amplitude + offset
    ...
    std_global = concatenated_dfs["y"].std()
    concatenated_dfs["y"] = (concatenated_dfs["y"]) / std_global
    # Applx global normalization to data without noise according to th std of the data with noise
    if calc_without_noise:
        concatenated_dfs["y_ideal"] = (concatenated_dfs["y_ideal"]) / std_global
